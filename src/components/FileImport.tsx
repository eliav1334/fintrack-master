                  ...
                  <div className="flex items-center space-x-4 gap-2">
                    <div>
                      ...
                      <span className="text-green-600">
                        {formatCurrency(previewData.filter(d => d.type === "income").reduce((sum, d) => sum + d.amount, 0))}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium">הוצאות: </span>
                      <span className="text-red-600">
                        {formatCurrency(previewData.filter(d => d.type === "expense").reduce((sum, d) => sum + d.amount, 0))}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {priceAlerts.length > 0 && (
                  <div className="bg-amber-50 dark:bg-amber-950/30 p-3 rounded-md border border-amber-200 dark:border-amber-800 mb-4">
                    <h4 className="font-medium text-amber-800 dark:text-amber-400 flex items-center mb-2">
                      ...
                      התראות על שינויי מחיר בתשלומים קבועים
                    </h4>
                    <ul className="list-disc pr-5 text-sm space-y-1 text-amber-700 dark:text-amber-300">
                ...
                <div className="overflow-auto border rounded-md">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-800">
                      <tr>
                        <th scope="col" className="px-3 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">תאריך</th>
                        <th scope="col" className="px-3 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">סוג</th>
                        <th scope="col" className="px-3 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">תיאור</th>
                        <th scope="col" className="px-3 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">סכום</th>
                        {previewData.some(d => d.category) && (
                          <th scope="col" className="px-3 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">קטגוריה</th>
                        )}
                        {previewData.some(d => d.cardNumber) && (
                          <th scope="col" className="px-3 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">כרטיס</th>
                        )}
                      </tr>
                    </thead>
                          ...
                          <td className="px-3 py-2 whitespace-nowrap text-sm">
                            <div className="flex items-center">
                              {transaction.type === "income" ? (
                                <ArrowUpCircle className="h-4 w-4 text-green-500 ml-2" />
                              ) : (
                                <ArrowDownCircle className="h-4 w-4 text-red-500 ml-2" />
                              )}
      ...
      <Dialog open={showNewFormatDialog} onOpenChange={setShowNewFormatDialog}>
        <DialogContent className="sm:max-w-[700px]">
          <DialogHeader>
            ...
            <DialogDescription>
              הגדר כיצד המערכת תנתח את הקובץ המיובא
            </DialogDescription>
          </DialogHeader>
          
          <div className="space-y-6 py-4">
            <div>
              <Label htmlFor="format-name">שם פורמט</Label>
              <Input
                id="format-name"
                value={newFormat.name}
                onChange={(e) => handleNewFormatChange(e, "name")}
                placeholder="שם לזיהוי הפורמט (לדוגמה: כרטיס אשראי ישראכרט)"
              />
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="mapping-date">שם עמודת תאריך</Label>
                  <Input
                    id="mapping-date"
                    value={newFormat.mapping.date}
                    onChange={(e) => handleNewFormatChange(e, "mapping", "date")}
                    placeholder="לדוגמה: תאריך עסקה"
                  />
                </div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => 
                      setNewFormat(prev => ({ ...prev, dateFormat: value }))
                    }
                  >
                ...
                <div>
                  <Label htmlFor="mapping-amount">שם עמודת סכום</Label>
                  <Input
                    id="mapping-amount"
                ...
                <div>
                  <Label htmlFor="mapping-description">שם עמודת תיאור</Label>
                  <Input
                    id="mapping-description"
                    value={newFormat.mapping.description}
                    onChange={(e) => handleNewFormatChange(e, "mapping", "description")}
                    placeholder="לדוגמה: פרטי עסקה"
                  />
                </div>
                
                <div>
                  <Label htmlFor="mapping-type">שם עמודת סוג (אופציונלי)</Label>
                  <Input
                    id="mapping-type"
                    value={newFormat.mapping.type || ""}
                    onChange={(e) => handleNewFormatChange(e, "mapping", "type")}
                    placeholder="לדוגמה: סוג עסקה"
                  />
                </div>
                
                <div>
                  <Label htmlFor="mapping-category">שם עמודת קטגוריה (אופציונלי)</Label>
                  <Input
                    id="mapping-category"
                ...
                <div>
                  <Label htmlFor="mapping-card-number">שם עמודת מספר כרטיס (אופציונלי)</Label>
                  <Input
                    id="mapping-card-number"
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => 
                      setNewFormat(prev => ({ ...prev, delimiter: value }))
                    }
                  >
                    <SelectTrigger id="delimiter">
                      <SelectValue placeholder="בחר תו מפריד" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value=",">פסיק (,)</SelectItem>
                      <SelectItem value=";">נקודה-פסיק (;)</SelectItem>
                      <SelectItem value="\t">Tab</SelectItem>
                      <SelectItem value="|">קו אנכי (|)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <h4 className="font-medium">זיהוי סוג עסקה</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="type-column">עמודה לזיהוי סוג עסקה</Label>
                  <Input
                    id="type-column"
                    value={newFormat.typeIdentifier.column}
                    onChange={(e) => handleNewFormatChange(e, "typeIdentifier", "column")}
                    placeholder="לדוגמה: סוג עסקה או חובה/זכות"
                  />
                </div>
                
                <div>
                  <Label htmlFor="income-values">ערכים שמייצגים הכנסה (מופרדים בפסיק)</Label>
                  <Input
                    id="income-values"
                    value={newFormat.typeIdentifier.incomeValues.join(",")}
                    onChange={(e) => handleArrayChange(e.target.value, "typeIdentifier", "incomeValues")}
                    placeholder='לדוגמה: "הכנסה,זכות,income,credit"'
                  />
                </div>
                
                <div className="sm:col-span-2">
                  <Label htmlFor="expense-values">ערכים שמייצגים הוצאה (מופרדים בפסיק)</Label>
                  <Input
                    id="expense-values"
                    ...
                    placeholder='לדוגמה: "הוצאה,חובה,expense,debit"'
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="example-structure">מבנה לדוגמה (אופציונלי)</Label>
              <Textarea
                id="example-structure"
                placeholder='לדוגמה: "תאריך עסקה,סכום,פרטי עסקה,סוג,קטגוריה"'
                className="h-24"
              />
              <p className="text-xs text-gray-500">
                תיאור קצר של מבנה הקובץ לעזרה בזיהוי הפורמט בעתיד
              </p>
            </div>
          </div>
